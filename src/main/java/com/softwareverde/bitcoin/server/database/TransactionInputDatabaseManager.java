...
public class TransactionInputDatabaseManager {
    ...
    public List<TransactionInputId> insertTransactionInputs(final List<TransactionId> transactionIds, final List<Transaction> transactions) throws DatabaseException {
        ...
        findPreviousTransactionsTimer.start();
        ...
        findPreviousTransactionsTimer.stop();
        ...
        final HashMap<StoredTransactionOutputIdentifier, TransactionOutputId> transactionOutputMap = new HashMap<StoredTransactionOutputIdentifier, TransactionOutputId>(transactionCount * 2);
        {
            ...
            final java.util.List<Row> rows = _databaseConnection.query(
                new Query("SELECT id, transaction_id, `index` FROM transaction_outputs WHERE is_spent = 0 AND transaction_id IN (" + DatabaseUtil.createInClause(previousOutputTransactionIds) + ")")
            );
            for (final Row row : rows) {
            ...
            for (final TransactionInput transactionInput : transactionInputs) {
                final TransactionOutputId previousTransactionOutputId;
                {
                    ...
                    if (Util.areEqual(Sha256Hash.EMPTY_HASH, previousOutputTransactionHash)) {
                        ...
                        {
                            final TransactionOutputId transactionOutputId = transactionOutputMap.get(transactionOutputIdentifier);
                            if (transactionOutputId != null) {
                                previousTransactionOutputId = transactionOutputId;
                            }
                            else {
                                final java.util.List<Row> rows = _databaseConnection.query(
                                    new Query("SELECT id, transaction_id, `index` FROM transaction_outputs WHERE is_spent = 1 AND transaction_id = ? AND `index`= ?")
                                        .setParameter(previousOutputTransactionId)
                                ...
                                final Row row = rows.get(0);
                                previousTransactionOutputId = TransactionOutputId.wrap(row.getLong("id"));
                            }
                        }
                    }
                }
