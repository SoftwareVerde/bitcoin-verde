...
public class TransactionRelay {
    ...
    protected void _relayTransactions(final List<Transaction> transactions) {
        final MutableMap<NodeId, MutableList<Sha256Hash>> nodeUnseenTransactionHashes = new MutableHashMap<>();
        final MutableList<TransactionWithFee> transactionsToAnnounceViaRpc = new MutableArrayList<>((_nodeRpcHandler != null) ? transactions.getCount() : 0);
            ...
            for (final Transaction transaction : transactions) {
                ...
                for (final NodeId nodeId : nodesWithoutTransaction) {
                    ...
                    if (! nodeUnseenTransactionHashes.containsKey(nodeId)) {
                        nodeUnseenTransactionHashes.put(nodeId, new MutableArrayList<>());
                    }
        ...
        for (final NodeId nodeId : nodeUnseenTransactionHashes.getKeys()) {
            final BitcoinNode bitcoinNode = _bitcoinNodeManager.getNode(nodeId);
            if (bitcoinNode == null) { continue; }
