...
public class StratumModuleTests extends UnitTest {
    ...
    public void should_mine_valid_prototype_block() {
        ...
        final InMemoryPropertiesStore inMemoryPropertiesStore = stratumServer.getPropertiesStore();
        inMemoryPropertiesStore.set(BitcoinCoreStratumServer.COINBASE_ADDRESS_KEY, "12c6DSiU4Rq3P4ZxziKxzrL5LmMBrzjrJX");
...
class BitcoinVerdeStratumServerPartialMock extends BitcoinVerdeStratumServer {
    ...
    public BitcoinVerdeStratumServerPartialMock() {
        super(CONFIGURATION.getStratumProperties(), new InMemoryPropertiesStore(), new CachedThreadPool(1, 1L), new CoreInflater());
        ((CachedThreadPool) _threadPool).start();

        ReflectionUtil.setValue(this, "_stratumServerSocket", new FakeStratumServerSocket());
    }

    public InMemoryPropertiesStore getPropertiesStore() {
        return (InMemoryPropertiesStore) _propertiesStore;
    }
